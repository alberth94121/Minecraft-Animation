...
//Axe Head
const int numAxeVertices = 18;

vec4 AxePoints [numAxeVertices];
vec3 AxeNormals[numAxeVertices];
vec2 AxeUV     [numAxeVertices];

// Vertices of a axe head
vec4 AxeVertices[5] = {
...
int i = 0;
void Axe(int a, int b, int c, const vec3& normal )
{
    AxePoints[i] = AxeVertices[a]; 
	AxeNormals[i] = normal; 
    AxeUV[i] = vec2(0.0f, 1.0f); 
	i++;
    AxePoints[i] = AxeVertices[b]; 
	AxeNormals[i] = normal;
    AxeUV[i] = vec2(0.0f, 0.0f);
	i++;
    AxePoints[i] = AxeVertices[c]; 
	AxeNormals[i] = normal;
    AxeUV[i] = vec2(1.0f, 0.0f); 
	i++;
}

void colorAxe()
{
    Axe(0, 2, 3, vec3(-1.0f,  1.0f,  -1.0f) );
    Axe(2, 0, 4, vec3(-1.0f,  1.0f,  1.0f) );
    Axe(1, 2, 3, vec3(1.0f, 1.0f,  -1.0f) );
    Axe(0, 1, 3, vec3(0.0f,  1.0f,  0.0f) );
	Axe(0, 1, 4, vec3(0.0f,  1.0f,  0.0f) );
	Axe(1, 2, 4, vec3(1.0f,  1.0f,  1.0f) );
	  
}

// initialization
void generateAxe(GLuint program, ShapeData* AxeData)
{
    colorAxe();
	AxeData->numVertices = numAxeVertices;

    // Create a vertex array object
    glGenVertexArrays(1, &AxeData->vao );
    glBindVertexArray(AxeData->vao );

    // Set vertex attributes
    setVertexAttrib(program, 
                   (float*)AxePoints,  sizeof(AxePoints), 
                   (float*)AxeNormals, sizeof(AxeNormals),
                   (float*)AxeUV,      sizeof(AxeUV));
}
